...
# ReadMemoryByAddress ($23) channels deferred to Phase 3 HSL implementation.
...
SIMOS85_CHANNELS: List[Channel] = [
    ...
    # These DID addresses are best-effort from community captures;
    # confirm against your specific bin via DID 0xF19E ASAM file.
...
class LogRow:
    ...
    wall_time:  str                      # HH:MM:SS.mmm
    values:     Dict[int, Optional[float]]  # did → decoded value
    raw:        Dict[int, bytes]         # did → raw bytes
...
class LogSession:
    ...
    def _loop(self):
        ...
        # Build codec map — all DIDs use raw bytes passthrough
            ...
            while self._running:
                ...
                for ch in self.channels:
                    try:
                        ...
                        values[ch.did] = None  # DID not supported
                    except Exception as e:
                        values[ch.did] = None
